...
function decToHexFromMultiBaseMath(dec) {
	return (new MultiBaseInteger(dec, 16).baseIntegerString);
}
...
function decToEffHex(dec) {
	return decToHex(decToEffDec(dec));
}
function decToEffHexFromMultiBaseMath(dec) {
	return decToHexFromMultiBaseMath(decToEffDec(dec));
}
...
function setEffNumberValueFromMultiBaseMath(targetNumberSelector, decimalValue) {
	var val = decToHexFromMultiBaseMath(decimalValue);
	$(targetNumberSelector).attr("eff-number-value", val);
	updateEffNumbers(targetNumberSelector);
}
